...
#include<cstdarg>  // C language is <stdarg.h>
...
{
	/*像这样初始化Shader*/
	mvsByteCode = d3dUtil::CompileShader(L"Shaders\\ScreenPassTest.hlsl", nullptr, "VS", "vs_5_0");
	mpsByteCode = d3dUtil::CompileShader(L"Shaders\\ScreenPassTest.hlsl", nullptr, "PS", "ps_5_0");
	mWindowCenter = Vector2(0, 0);
	mWindowScale = Vector2(1, 1);
}

ScreenPass::ScreenPass(const string& ShaderFilePath, const Vector2& WindowCenter, const Vector2& WindowScale)
{
	StringHelper FStringHelper;
	mvsByteCode = d3dUtil::CompileShader(FStringHelper.StringToWString(ShaderFilePath), nullptr, "VS", "vs_5_0");
	mpsByteCode = d3dUtil::CompileShader(FStringHelper.StringToWString(ShaderFilePath), nullptr, "PS", "ps_5_0");
...
void ScreenPass::Draw(ID3D12GraphicsCommandList* mCommandList, ID3D12CommandAllocator* mDirectCmdListAlloc, RenderTarget* destRT)
{
	...
	mScissorRect = { 0, 0, (long)destRT->GetWidth(), (long)destRT->GetHeight() };
...
{
	//Build Screen Geometry data直接就是屏幕空间的
...
{
	/*this is a simple PSO and you should override it*/
...
{
	/*this is a simple RootSignature and you should override it*/
	//没有资源输入的RootSignature,直接从Shader输出数据到RT上
	{
	...
	//绑定一个带一张图的RootSignature
	{
		...
		ThrowIfFailed(D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error));
		ThrowIfFailed(md3dDevice->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&mRootSignatureB)));
	}
}

std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> ScreenPass::GetStaticSamplers()
{
